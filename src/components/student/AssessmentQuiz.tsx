...
const AssessmentQuiz: React.FC = () => {
  ...
  const handleCompleteQuiz = async () => {
    ...
    try {
      // Generate a roadmap based on the assessment answers
      const assessmentResults = Object.entries(answers).map(([questionId, answer]) => {
        const question = initialAssessmentQuiz.questions.find(q => q.id === questionId);
        return {
          topic: question?.topic,
          difficulty: question?.difficulty,
          answer,
          correct: question?.type === 'short_answer' 
            ? question.correctAnswer.some(ans => answer.toLowerCase().includes(ans.toLowerCase()))
            : question?.correctAnswer.includes(answer)
        };
      });

      // Calculate student's proficiency by topic
      const topicProficiency = assessmentResults.reduce((acc, result) => {
        if (!result.topic) return acc;
        if (!acc[result.topic]) {
          acc[result.topic] = { total: 0, correct: 0, avgDifficulty: 0 };
        }
        acc[result.topic].total++;
        if (result.correct) acc[result.topic].correct++;
        acc[result.topic].avgDifficulty = (acc[result.topic].avgDifficulty + (result.difficulty || 0)) / 2;
        return acc;
      }, {} as Record<string, { total: number; correct: number; avgDifficulty: number; }>);

      // Navigate to roadmap with assessment data
      navigate(`/student/courses/${courseId}/roadmap`, {
        state: {
          assessmentData: {
            topicProficiency,
            completedAt: new Date().toISOString(),
            overallScore: assessmentResults.filter(r => r.correct).length / assessmentResults.length,
            recommendedStartingLevel: Math.round(
              assessmentResults.reduce((acc, r) => acc + (r.difficulty || 0), 0) / assessmentResults.length * 10
            ) / 10
          }
        }
      });
      ...
      toast.error('Failed to generate your roadmap. Please try again.');
    }
  };
